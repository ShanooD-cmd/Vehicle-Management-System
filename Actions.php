...
try
{
	...
	$con = mysqli_connect("localhost","root","1111");   //change as per your localhost or server database settings
	mysqli_select_db($con,"plusplusGroup");   //change as per your localhost or server database settings
	...
	else if($_GET["action"] == "create")
	{
		...
		$result = mysqli_query($con,"INSERT INTO manufacturer(manufacturer_name) VALUES('" . $_POST["manufacturer_name"] . "');");
	...
	else if($_GET["action"] == "update")
	{
		...
		$result = mysqli_query($con,"UPDATE manufacturer SET manufacturer_name = '" . $_POST["manufacturer_name"] . "' WHERE manufacturer_id = " . $_POST["manufacturer_id"] . ";");
	...
	{
		//Insert record into database
		$result = mysqli_query($con,"INSERT INTO model(model_name, manufacturer_name) VALUES('" . $_POST["model_name"] . "', '" . $_POST["manufacturer_name"] . "');");
	...
	{
		//Update record in database
		$result = mysqli_query($con,"UPDATE model SET model_name = '" . $_POST["model_name"] . "', manufacturer_name = '" . $_POST["manufacturer_name"] . "' WHERE model_id = " . $_POST["model_id"] . ";");
...
<body>
  <div class="modal-dialog modal-lg">
	...
	if(mysqli_query($con,"DELETE FROM customer WHERE v_id = $id;"))
		{   
		$result = mysqli_query($conn,"UPDATE vehicle SET s_price = NULL,u_id = NULL,sold_date = NULL, status = 'Available'  WHERE v_id =$id;");
			?>
